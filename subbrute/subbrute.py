...
import sys
import os
sys.path.append(os.path.abspath("./subbrute"))
import dnslib
import re
import optparse
import signal
...
def print_target(target, query_type = "ANY", subdomains = "subbrute/names.txt", resolve_list = "resolvers.txt", process_count = 16, print_data = False, output = False, json_output = False):
    json_struct = {}
    if not print_data:
        dupe_filter = {}
...
def run(target, query_type = "ANY", subdomains = "subbrute/names.txt", resolve_list = False, process_count = 8):
    print("Subbrute run")
    spider_blacklist = {}
    result_blacklist = {}
    found_domains = {}
...
if __name__ == "__main__":
    ...
    parser.add_option("-s", "--subs", dest = "subs", default = os.path.join(base_path, "subbrute/names.txt"),
              type = "string", help = "(optional) A list of subdomains, accepts a single file, or a directory of files. default = 'subbrute/names.txt'")
